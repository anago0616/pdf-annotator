...
async function renderAllPages() {
  ...
  for (let num = 1; num <= state.pdf.numPages; num++) {
    ...
    // 実効解像度: 端末の上限(面積・1辺)を絶対に超えないようにしつつ、できるだけ高精細に
    ...
    if (state.zoom < 4) dpr = Math.max(baseDpr, dpr); // 低〜中倍率は端末DPR以上を確保(鮮明)
    dpr = Math.min(dpr, hardCap); // 上限は最後に必ず適用(超倍率では1未満まで下げてブランク/ぼやけを防ぐ)
...
/* ---- ズーム ---- */
const ZOOM_MIN = 0.5, ZOOM_MAX = 16; // 最大16倍(高倍率は端末上限内に自動調整)
...
const _receivingDocs = new Set(); // 取得中の remoteId を記録して二重取得を防ぐ

// doc:add通知・init補完・joinFolder共通: 1件取得(60秒タイムアウト・2回リトライ・二重取得防止)
async function receiveSharedDoc(shareCode, remoteId, { silent = false } = {}) {
  const key = shareCode + ':' + remoteId;
  if (_receivingDocs.has(key)) return false; // 既に取得中
  _receivingDocs.add(key);
  try {
    const all = await dbAll();
    if (all.some(d => d.shareCode === shareCode && d.remoteId === remoteId)) return true; // 既存
    ...
    for (let attempt = 0; attempt <= 2; attempt++) {
      ...
      try {
        ...
        await dbPut({
          ...
          pdfData, annotations: data.annotations || {},
          pageCount: pdf.numPages, shareCode, remoteId, updatedAt: Date.now()
        });
        await pdf.destroy();
        if (!silent) showToast(`共有文書「${data.name}」が追加されました`);
        if (!views.home.hidden) renderHome();
        return true;
      } catch (err) {
        clearTimeout(timer);
        if (attempt < 2) await new Promise(r => setTimeout(r, 3000 * (attempt + 1)));
        else console.error('共有文書の受信に失敗(リトライ上限):', remoteId, err);
      }
    }
    return false;
  } finally {
    _receivingDocs.delete(key);
  }
}
...
async function ensureConnections() {
  ...
  for (const code of codes) {
    const c = conns.get(code);
    if (c && c.recreating) continue; // recreateRoom進行中は干渉しない
...
async function joinFolder(codeInput) {
  ...
  let added = 0, fail = 0;
  // receiveSharedDoc に統一することで init との二重取得を防ぐ
  for (const remoteId of toFetch) {
    const ok = await receiveSharedDoc(code, remoteId, { silent: true });
    if (ok) added++; else fail++;
...
/* ---- インストールバナー ---- */
{
  ...
  // ✕ボタン・iOS閉じるボタン: 常に登録(IIFE早期returnで登録漏れしないよう外に出す)
  ...
  // Android/Chrome: インストール可能になったらバナー表示
  window.addEventListener('beforeinstallprompt', (e) => {
    e.preventDefault();
    deferredPrompt = e;
    if (!isStandalone() && !localStorage.getItem('pdfnote_install_dismissed')) {
      $('installBannerMsg').textContent = 'アプリとしてインストールできます';
  ...
  // iOS Safari: スタンドアロンでなければバナー表示
  if (isIOS && isSafari && !isStandalone() && !localStorage.getItem('pdfnote_install_dismissed')) {
    $('installBannerMsg').textContent = 'ホーム画面に追加してアプリとして使えます';
